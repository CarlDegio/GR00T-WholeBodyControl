"""Regression tests copied from the agent-near heading feedback contract."""
...
def test_orientation_tracker_reports_heading_and_wrapped_lag() -> None:
    ...
    sample = tracker.sample(10.12, 0.15)
...
def test_orientation_tracker_without_state_keeps_actual_fields_null() -> None:
    sample = OrientationTracker().sample(2.0, 0.2)
...
def test_orientation_tracker_wraps_lag_across_pi_boundary() -> None:
    ...
    sample = tracker.sample(1.1, math.pi - 0.05)

    assert sample.heading_lag_rad == pytest.approx(-0.1)


def test_orientation_telemetry_round_trip_and_latest_sample_age() -> None:
    older = OrientationTelemetrySample(10.0, 0.4, 0.1, 0.15, 0.05, 0.01)
    newer = OrientationTelemetrySample(10.2, 0.5, 0.2, 0.25, 0.05, 0.02)
    ...
    assert decode_orientation_telemetry(encode_orientation_telemetry(newer)) == newer
    assert latest.diagnostics(10.3) == pytest.approx(
        {
            "actual_yaw_rad": 0.5,
