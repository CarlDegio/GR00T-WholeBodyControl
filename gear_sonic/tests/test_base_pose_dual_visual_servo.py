...
def test_coordinator_uses_alternate_initial_then_origin_latest_then_stops() -> None:
    head_initial = reference(HEAD, marker=1)
    chest_initial = reference(CHEST, marker=2)
    head_latest = reference(HEAD, kind="latest", marker=9)
    coordinator = DualCameraFailoverCoordinator(
        (HEAD, CHEST),
    ...
    coordinator.save_latest(head_latest)

    alternate = coordinator.advance_after_failure(first)
    assert alternate is not None
    assert alternate.live_stream == CHEST
    assert alternate.reference is chest_initial
    assert alternate.stage == "alternate_initial"
    assert alternate.origin_stream == HEAD

    retry = coordinator.advance_after_failure(alternate)
    assert retry is not None
    assert retry.live_stream == HEAD
    assert retry.reference is head_latest
    assert retry.stage == "origin_latest"
    assert retry.origin_stream == HEAD

    assert coordinator.advance_after_failure(retry) is None


def test_coordinator_uses_cross_camera_initial_when_alternate_has_none() -> None:
    head_initial = reference(HEAD, marker=1)
    coordinator = DualCameraFailoverCoordinator(
        (HEAD, CHEST),
        {HEAD: head_initial},
    ...
    assert alternate is not None
    assert alternate.live_stream == CHEST
    assert alternate.reference is head_initial
    assert alternate.reference.stream_name == HEAD
    assert alternate.stage == "alternate_initial"


def test_coordinator_origin_retry_falls_back_to_origin_initial() -> None:
    head_initial = reference(HEAD, marker=1)
    coordinator = DualCameraFailoverCoordinator(
        (HEAD, CHEST),
        {HEAD: head_initial},
    ...
    assert retry is not None
    assert retry.live_stream == HEAD
    assert retry.reference is head_initial
    assert retry.stage == "origin_latest"
...
def test_coordinator_success_resets_cycle_for_repeated_switching() -> None:
    ...
    next_head = coordinator.advance_after_failure(chest)

    assert next_head is not None
    assert next_head.live_stream == HEAD
    assert next_head.reference is head_initial
    assert next_head.stage == "alternate_initial"
    assert next_head.origin_stream == CHEST
    assert next_head.attempt_id == 3
...
def test_worker_gives_each_attempt_thirty_frames_before_three_stage_exit(
    tmp_path: Path,
) -> None:
    ...
    assert len(invalid) == 87
    assert [event.details["match_invalid_frames"] for event in invalid[28::29]] == [29, 29, 29]
    assert len(switching) == 2
    assert [event.details["live_stream"] for event in switching] == [CHEST, HEAD]
    assert tracker.starts == [1, 2, 1]
    assert len(errors) == 1
    assert "three-stage failover exhausted" in (errors[0].error or "")
    diagnostic_indices = [
        event.frame.frame_index for event in emitted if event.frame is not None
    ]
    assert diagnostic_indices == list(range(90))
    ...
    assert camera.capture_count == 91
...
def test_worker_switch_boundaries_flush_contiguous_async_diagnostics(
    ...
    assert recorded_indices == list(range(90))
...
def _valid_instances() -> list[TrackedInstance]:
    ...
    return [
        ...
        TrackedInstance(2, 1, "table", 0.8, (100.0, 120.0, 540.0, 360.0), table_mask),
    ]
...
def test_worker_origin_retry_uses_fifth_frame_latest_reference(tmp_path: Path) -> None:
    stop_event = threading.Event()

    class LatestThenMissingTracker:
        def __init__(self) -> None:
            self.attempt = 0
            self.calls = 0
            self.starts: list[int] = []
        ...
        def track(self, _rgb):
            self.calls += 1
            if self.attempt == 1 and self.calls <= 5:
                return _valid_instances()
            return []

    tracker = LatestThenMissingTracker()
    ...
    assert len(initialized) == 1
    assert len(switching) == 2
    assert tracker.starts[:2] == [1, 2]
    assert tracker.starts[2] > 2
    assert switching[1].details["reference_kind"] == "latest"
    assert switching[1].details["reference_source_stream"] == HEAD
...
def test_diagnostics_record_dual_camera_attempt_provenance(tmp_path: Path) -> None:
    ...
    frame = DetectionFrameData(
        ...
        camera_stream=CHEST,
        attempt_id=2,
        failover_stage="alternate_initial",
    ...
    assert row["failover_stage"] == "alternate_initial"
