...
from gear_sonic.runtime.sensor_gateway import DEPTH_ANYTHING_STATUS_TYPE
...
class DepthAnythingConfig:
    ...
    ready_file: str = ""
    depth_anything_root: str = "/home/user/Project/Depth-Anything-V2/metric_depth"
    checkpoint: str = (
        "/home/user/Project/Depth-Anything-V2/metric_depth/checkpoints/"
        "depth_anything_v2_metric_hypersim_vitb.pth"
    )
    camera_intrinsics_path: str = str(DEFAULT_CAMERA_INTRINSICS_PATH)
    encoder: str = "vitb"
    device: str = "cuda"
    inference_hz: float = 10.5
    input_size: int = 644
    model_max_depth_m: float = 20.0
    publish_max_depth_m: float = 20.0
    use_amp: bool = True
    idle_heartbeat_hz: float = 2.0
    ...
    def read(self) -> tuple[np.ndarray, float, dict[str, Any]] | None:
        try:
            ...
            if message != self._last_error or now - self._last_error_time >= 2.0:
                print(f"[DepthAnything] waiting for chest RGB: {message}")
...
class DepthAnythingMetricEstimator:
    ...
    def __init__(self, config: DepthAnythingConfig) -> None:
        root = Path(config.depth_anything_root).expanduser().resolve()
        ...
        print(f"[DepthAnything] loading {checkpoint} on {self.device} ...")
    ...
    def infer(self, rgb: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        return depth.float().cpu().numpy(), time.monotonic() - started
...
def main(config: DepthAnythingConfig) -> None:
    ...
    if config.ready_file:
        ...
        print(f"[DepthAnything] ready marker: {config.ready_file}")
    ...
    try:
        while True:
            ...
            if state_changed:
                print(
                    f"[DepthAnything] inference "
                    f"{'active' if gate.active else 'idle'}; "
                    f"owner={gate.owner or 'none'} generation={gate.generation}"
                )
            ...
            if packet is not None:
                ...
                depth_m, inference_seconds = estimator.infer(rgb)
                ...
                if count == 1 or count % 100 == 0:
                    print(
                        f"[DepthAnything] published chest metric depth #{count}; "
                        f"inference={inference_seconds * 1000.0:.1f}ms"
                    )
...
if __name__ == "__main__":
    main(tyro.cli(DepthAnythingConfig))
