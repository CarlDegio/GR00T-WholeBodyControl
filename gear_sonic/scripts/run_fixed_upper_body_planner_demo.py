...
Usage:
  python gear_sonic/scripts/run_fixed_upper_body_planner_demo.py
  python gear_sonic/scripts/run_fixed_upper_body_planner_demo.py --publisher   # tmux worker
  python gear_sonic/scripts/run_fixed_upper_body_planner_demo.py --skip-sim --no-attach
"""
...
#                  (symmetrized pose from LATENT_INITIAL_MOTION_TOKEN / run_vla_inference)
...
def run_launcher(config: Config) -> None:
    ...
    script = "gear_sonic/scripts/run_fixed_upper_body_planner_demo.py"
    pub_cmd = (
        f"cd {REPO_ROOT} && source .venv_teleop/bin/activate && "
        f"python {script} --publisher "
