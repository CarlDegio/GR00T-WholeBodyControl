...
class BasePoseResult:
    ...
    prompt: str = ""
    depth_query_selection: dict[str, Any] | None = None
    depth_measurements: list[dict[str, Any]] | None = None
    backend: str = ""
...
) -> CodexStructuredVisionClient | QwenVLStructuredVisionClient:
    ...
    def __init__(
        ...
        *,
        ...
        if client is not None:
            ...
            self.client = _build_vision_client(config)
...
""" + json.dumps(
            ...
            return BasePoseResult(
                ...
                timing_s=timing,
                raw_output=dict(raw_plan),
                prompt=prompt,
                depth_query_selection=selection,
...
BasePoseObservation = AlignedRGBDSnapshot


def validate_base_pose_observation(
    config: BasePoseConfig, observation: BasePoseObservation
) -> None:
    """Validate caller-owned observations before any model request is made."""

    rgb = np.asarray(observation.rgb)
    if rgb.ndim != 3 or rgb.shape[2] != 3 or rgb.dtype != np.uint8:
        raise BasePoseCameraError(
            f"base-pose RGB must be HxWx3 uint8, got {rgb.shape} {rgb.dtype}"
        )
    calibration = (observation.fx, observation.fy, observation.cx, observation.cy)
    if not all(math.isfinite(float(value)) for value in calibration):
        raise BasePoseCameraError("base-pose camera intrinsics must be finite")
    if observation.fx <= 0.0 or observation.fy <= 0.0:
        raise BasePoseCameraError("base-pose fx and fy must be positive")
    if config.mode == "rgb":
        return
    if observation.depth_raw is None:
        raise BasePoseCameraError(f"{config.mode} requires aligned uint16 depth")
    depth = np.asarray(observation.depth_raw)
    if depth.ndim != 2 or depth.dtype != np.uint16:
        raise BasePoseCameraError(
            f"aligned depth must be HxW uint16, got {depth.shape} {depth.dtype}"
        )
    if depth.shape != rgb.shape[:2]:
        raise BasePoseCameraError("aligned RGB and depth shapes do not match")
    if observation.depth_aligned_to != config.camera_stream:
        raise BasePoseCameraError("depth is not aligned to the requested RGB stream")
    if observation.depth_source != "lingbot-depth":
        raise BasePoseCameraError("depth_source must be 'lingbot-depth'")
    if observation.depth_scale_m is None or not math.isclose(
        observation.depth_scale_m, 0.001, rel_tol=0.0, abs_tol=1.0e-9
    ):
        raise BasePoseCameraError(
            "LingBot enhanced uint16 depth must use millimeter units"
        )
...
class _SingleObservationCamera:
    def __init__(self, observation: BasePoseObservation) -> None:
        self.observation = observation
        self._captured = False

    def capture(self) -> BasePoseObservation:
        if self._captured:
            raise BasePoseCameraError("the supplied base-pose observation was already used")
        self._captured = True
...
class BasePosePlanner:
    """Plan from caller-provided observations without owning a sensor transport."""
    ...
    def plan(self, observation: BasePoseObservation) -> BasePoseResult:
        validate_base_pose_observation(self.config, observation)
        if self.client is None:
            self.client = _build_vision_client(self.config)
        if self.config.persist_diagnostics:
            runner = BasePoseRunner(
                self.config,
                camera=_SingleObservationCamera(observation),  # type: ignore[arg-type]
                client=self.client,
            ...
            try:
                ...
                runner.close()
        with tempfile.TemporaryDirectory(prefix="sonic_base_pose_") as temporary:
            ephemeral = replace(self.config, output_root=temporary)
            runner = BasePoseRunner(
                ephemeral,
                camera=_SingleObservationCamera(observation),  # type: ignore[arg-type]
                client=self.client,
            ...
            try:
                ...
                runner.close()
            return replace(result, output_dir=None)

    def close(self) -> None:
        """Release a reusable Qwen client when one is owned by the planner."""

        inner_client = getattr(self.client, "client", None)
        close = getattr(inner_client, "close", None)
        if callable(close):
            close()
