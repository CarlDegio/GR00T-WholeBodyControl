"""ObjectNav perception, diagnostics, and SONIC JSON client support."""
...
EMPTY_COMMANDS: dict[str, list[Any]] = {"commands": []}
...
class SonicPlannerRequestError(RuntimeError):
    """Raised when the planner rejects or aborts an ObjectNav batch."""
...
class ComposedRGBDCamera:
    """Read fresh aligned chest RGB-D frames from the repository camera server."""
    ...
    def decode_payload(cls, payload: Mapping[str, Any]) -> RGBDSnapshot:
        ...
        decoded = ImageMessageSchema.deserialize(dict(payload), decode_images=True).asdict()
        ...
        if (width, height) != (rgb.shape[1], rgb.shape[0]):
            raise ObjectNavCameraError("camera_info dimensions do not match chest images")

        return RGBDSnapshot(
            rgb_bgr=cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR),
            depth_raw=depth_raw,
            depth_mm=depth_raw.astype(np.float32) * np.float32(numeric["depth_scale_m"] * 1000),
            fx=numeric["fx"],
            fy=numeric["fy"],
            cx=numeric["cx"],
            cy=numeric["cy"],
    ...
    def capture_aligned_rgbd(self, timeout_ms: int | None = None) -> RGBDSnapshot:
        ...
        while True:
            ...
            try:
                ...
                raise ObjectNavCameraError("failed to decode composed-camera message") from exc
            if self._last_timestamp is None or snapshot.timestamp != self._last_timestamp:
                self._last_timestamp = snapshot.timestamp
                return snapshot
            if time.monotonic() >= deadline:
                raise ObjectNavCameraError("timed out waiting for a newer chest RGB-D frame")
...
class CodexBBoxClient:
    ...
    def _valid_bbox(cls, value: Any) -> bool:
        if not isinstance(value, list) or len(value) != 4:
            return False
        if not all(cls._is_finite_number(item) and 0 <= float(item) <= 1000 for item in value):
            return False
    ...
    def validate_policy(cls, policy: Any) -> dict[str, Any]:
        ...
        if any(not isinstance(policy[key], str) for key in ("visual_check", "target", "stop_reasoning")):
            raise ValueError("Codex policy text fields must be strings")
        for key in ("confidence", "distance_confidence"):
            if not cls._is_finite_number(policy[key]) or not 0 <= float(policy[key]) <= 1:
        ...
        for key in ("horizontal_offset_pixel", "camera_bearing_deg"):
            if policy[key] is not None and not cls._is_finite_number(policy[key]):
                raise ValueError(f"Codex policy {key} is invalid")
        if policy["rotation_direction"] is not None and policy["rotation_direction"] not in ROTATION_DIRECTIONS:
            raise ValueError("Codex policy rotation_direction is invalid")
        angle = policy["rotation_angle_deg"]
        if angle is not None and (not cls._is_finite_number(angle) or float(angle) < 0):
    ...
    def locate(
        ...
        *,
        ...
        if not self.schema_path.is_file():
            raise FileNotFoundError(f"ObjectNav output schema not found: {self.schema_path}")
        command = [
            self.codex_bin,
            "--ask-for-approval",
            "never",
...
def _save_raw_depth_outputs(snapshots: list[RGBDSnapshot], output_dir: Path) -> None:
    frame_info = []
...
class ObjectNavRunner:
    """Reusable one-cycle ObjectNav runner used by both CLIs."""
    ...
    def __init__(
        ...
        *,
        ...
        policy_client: CodexBBoxClient | None = None,
    ):
        ...
        self.policy_client = policy_client or CodexBBoxClient(
            timeout_seconds=config.codex_timeout_seconds
        )

    def run_once(self, *, iteration: int = 1) -> ObjectNavResult:
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        output_dir = Path(self.config.output_root) / f"{timestamp}_{os.getpid()}" / f"iteration_{iteration:04d}"
        ...
        outcome = "FAILED"
        try:
            snapshots = [self.camera.capture_aligned_rgbd() for _ in range(FRAME_COUNT)]
            ...
            if float(policy["confidence"]) < self.config.min_confidence:
                ...
                outcome = "STOP"
                geometry = {"status": "stopped", "reason": policy["stop_reasoning"]}
            else:
            ...
            _write_json(output_dir / "object_nav_commands.json", commands, compact=True)
        ...
        if outcome == "NAVIGATE":
            ...
            print(
                ...
                f"rotation {prediction['rotation_direction']} {prediction['rotation_angle_deg']} "
                f"vs {measured['rotation_angle_deg']}deg",
                file=os.sys.stderr,
            )
...
def send_object_nav_commands(
    ...
    *,
    ...
    """Send one batch and wait for completed/rejected/aborted."""
    if not host:
    ...
    try:
        ...
        if not socket.poll(int(timeout_ms)):
            raise SonicPlannerRequestError(
                f"Sonic planner request timed out after {timeout_ms} ms"
            )
        reply = socket.recv_json()
        if not isinstance(reply, dict):
            raise SonicPlannerRequestError("Sonic planner returned an invalid reply")
        if reply.get("status") == "completed":
            return reply
        if reply.get("status") in {"rejected", "aborted"}:
            reason = reply.get("reason", "unspecified")
            detail = f" ({reply['detail']})" if reply.get("detail") else ""
            raise SonicPlannerRequestError(
                f"Sonic planner {reply['status']}: {reason}{detail}"
            )
        raise SonicPlannerRequestError("Sonic planner returned an invalid reply")
    except SonicPlannerRequestError:
        raise
    except Exception as exc:
        raise SonicPlannerRequestError(f"Sonic planner transport failed: {exc}") from exc
    finally:
        socket.close()
