...
Qwen grounds only the task target. YOLOE initializes that target from its box
and detects the supporting surface with a configured text prompt.
All motion after that point is generated by a bounded, deterministic visual
servo using persistent YOLOE track IDs and aligned RealSense depth.
"""
...
DEFAULT_SURFACE_TEXT_PROMPT = "desk"
...
class YoloePersistentTracker:
    ...
    def __init__(
        ...
        *,
        ...
        imgsz: int = 640,
        device: str = "0",
        surface_prompt: str = DEFAULT_SURFACE_TEXT_PROMPT,
    ):
        ...
        self.device = device
        self.surface_prompt = str(surface_prompt).strip()
        if not self.surface_prompt:
            raise ValueError("YOLOE surface text prompt must be non-empty")
    ...
    def start_target(
        ...
        *,
        ...
        """Install a visual target plus the configured text surface."""
        import torch
        from ultralytics.models.yolo.yoloe import YOLOEVPSegPredictor

        target = str(target_prompt).strip()
        if not target:
            raise ValueError("YOLOE target prompt must be non-empty")
        ...
        surface_embedding = self.model.get_text_pe([self.surface_prompt])
    ...
    *,
    ...
    for segment in segments:
        ...
        if (
            ...
            < _TABLE_EDGE_MIN_VALID_DEPTH_SAMPLES
        ):
            continue
    ...
    if best is None:
        raise ValueError(
            "no RGB-mask table-edge line has at least "
            f"{_TABLE_EDGE_MIN_VALID_DEPTH_SAMPLES} of "
            f"{_TABLE_EDGE_DEPTH_SAMPLES} valid sampled depths"
        )
...
def _dilate_table_edge_mask(mask: np.ndarray) -> np.ndarray:
    """Expand the cleaned desk mask to retain edges just outside its logits."""
    binary = (np.asarray(mask) > 0).astype(np.uint8)
    if binary.ndim != 2:
        raise ValueError("table mask must be two-dimensional")
    radius = _TABLE_EDGE_MASK_DILATION_RADIUS_PX
...
def estimate_table_geometry(
    ...
    *,
    ...
    if edge_intersection is None:
        binary = _dilate_table_edge_mask(binary)
        target_exclusion = (
            None
    ...
    def _update_joint_completion(
        ...
        *,
        ...
        if not lateral_stable:
            ...
            self.current = ServoCommand(0.0, vy, 0.0, self.command_ttl_s)
            return self.current
...
def build_servo_velocity_message(
    ...
    *,
    ...
    viewer_table_camera_stream: str | None = None,
) -> str:
    payload = {
        "action": action,
    ...
    if viewer_table_camera_stream:
        overlay["table_camera_stream"] = str(
            viewer_table_camera_stream
        )
...
def _surface_geometry_components(
    snapshot: AlignedRGBDSnapshot,
    surface: TrackedInstance,
    calibration: RawServoCalibration,
    *,
    target_mask: np.ndarray | None,
) -> tuple[TableGeometry | None, str | None, np.ndarray, np.ndarray | None]:
    """Extract desk yaw independently from whether a target is visible."""
    desk_mask = _dilate_table_edge_mask(
        _largest_filled_component(surface.mask)
    )
    ...
    try:
        target_exclusion = (
            None
            if target_mask is None
            else _table_edge_target_exclusion(target_mask, desk_mask.shape)
        )
        ...
        return None, str(exc), desk_mask, table_rgb_edges
    return table, None, desk_mask, table_rgb_edges
    ...
    *,
    ...
    if observation is not None:
        table_pixels_match_frame = (
            observation.table_camera_stream is None
            or camera_stream is None
            or observation.table_camera_stream == camera_stream
        )
        target_geometry = {
    ...
    return DetectionFrameData(
        ...
        table_rgb_edges=(
            ...
            if (
                ...
                or observation.table_rgb_edges is None
            )
        ...
        completed_surface_mask=(
            ...
            if (
                observation is None
                or not table_pixels_match_frame
                or observation.desk_mask is None
            )
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                ...
                visual_prompt = tracker.start_target(
                    snapshot.rgb,
                ...
                if surface is None:
                    raise RuntimeError(
                        "YOLOE found no instance for surface prompt "
                        f"{surface_prompt!r}"
                    )
                ...
                while gate.is_active(generation) and not stop_event.is_set():
                    ...
                    if text_recovery_active:
                        ...
                        visual_artifact = tracker.start_target(
                            snapshot.rgb,
                            target_prompt=spec.target_prompt,
                        ...
                        event_details.update(
                            {
                                ...
                                "text_surface_prompt": surface_prompt,
...
class RawServoRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.viewer_table_camera_stream: str | None = None
    ...
    def _event_camera_stream(details: Mapping[str, Any]) -> str | None:
        value = (
            details.get("live_stream")
            or details.get("camera_stream")
            or details.get("stream")
        )
        if value is None:
            return None
    ...
    def _target_distance_for_stream(self, stream: str | None) -> float:
        ...
        if (
            ...
            and stream == chest_stream
        ):
            return float(self.config.raw_chest_target_distance_m)
        return float(self.config.raw_head_target_distance_m)
    ...
    def _head_monitor_hold_requested(
        self,
        details: Mapping[str, Any],
    ) -> bool | None:
        """Return the head-monitor hold state carried by a chest event."""

        monitor = details.get("head_monitor")
        if not isinstance(monitor, Mapping):
            return None
        explicit = monitor.get("hold_active")
        if isinstance(explicit, bool):
            return explicit
        try:
            return int(monitor.get("target_streak", 0)) > 0
        except (TypeError, ValueError):
            return False
    ...
    def _apply_head_monitor_hold(
        ...
        if (
            ...
            self.head_monitor_hold_active = False
            return command
        requested = self._head_monitor_hold_requested(details)
        ...
        if requested:
            ...
            missing_frames = int(
                getattr(self.config, "dual_head_release_missing_frames", 3)
            )
            self._record("head_monitor_hold_released", details=dict(details))
            self.logger(
    ...
    def _clear_viewer_overlay(self) -> None:
        ...
        self.viewer_table_camera_stream = None
    ...
    def _set_viewer_overlay(
        ...
        self.viewer_table_camera_stream = (
            observation.table_camera_stream
            or yaw_stream
            or target_stream
        )
    ...
    def _set_viewer_overlay_from_frame(
        ...
        if frame.table_geometry is not None:
            raw_edge = frame.table_geometry.get("line_endpoints_px")
            if raw_edge is not None:
                self.viewer_table_edge_endpoints_px = tuple(
                    tuple(float(value) for value in point)
        ...
        desk_mask = (
            ...
            if frame.completed_surface_mask is not None
            else frame.surface_mask
            if yaw_uses_frame
            else None
        )
        if self.viewer_table_edge_endpoints_px is not None or desk_mask is not None:
            self.viewer_table_camera_stream = target_stream
        if desk_mask is not None:
            expected_shape = frame.rgb.shape[:2]
            if desk_mask.shape != expected_shape:
                raise ValueError(
                    "desk mask shape does not match viewer image size"
                )
            self.viewer_desk_mask_row_spans = _binary_mask_row_spans(
        ...
        if any(
            (
                ...
                self.viewer_table_edge_endpoints_px is not None,
                bool(self.viewer_desk_mask_row_spans),
            )
        ):
            self.viewer_image_size = (
    ...
    def _publish(self, command: ServoCommand, action: str) -> None:
        ...
        # target/table stream so mixed head/chest inputs render separately.
        active_camera_stream = self.active_camera_stream
        if (
    ...
    def accept_event(self, event: RawServoEvent, *, now: float) -> bool:
        ...
        if event.kind == HEAD_MONITOR_HOLD_EVENT:
            ...
            if (
                self.config.mode != "dual_raw_yoloe_servo"
                or self.phase not in {"inference", "aligning"}
                or self.active_camera_stream != chest_stream
            ...
            self.next_publish_at = float(now) + 1.0 / self.config.planner_hz
            self._record("head_monitor_hold_started", details=details)
            self.logger(
