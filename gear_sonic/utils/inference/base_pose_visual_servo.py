...
YOLOE detects both the configured target and supporting surface from fixed text
prompts. Motion is generated by a bounded, deterministic visual servo using
    ...
    if observation is not None:
        ...
        if observation.table is not None:
            table_geometry = {
                ...
                "line_length_px": observation.table.line_length_px,
                "valid_depth_samples": observation.table.valid_depth_samples,
                "line_center_px": list(observation.table.line_center_px),
                "line_endpoints_px": (
                    None
                    if (
...
class RawServoRuntime:
    ...
    def accept_event(self, event: RawServoEvent, *, now: float) -> bool:
        ...
        if event.kind in {"initialized", "observation"}:
            ...
            if initializing:
                self._record("initialized", details=details)
        ...
        if entered_post_stop_sampling:
            ...
            self._record(
                ...
                sample_frames=self.controller.post_stop_sample_frames,
                deviation_frames=self.controller.post_stop_deviation_frames,
                errors=self.controller.last_errors,
                orientation=orientation,
            )
            self.logger(
            ...
            self.next_publish_at = float(now) + 1.0 / self.config.planner_hz
            self._record(
                "post_stop_navigation_resumed",
                errors=self.controller.last_errors,
                consecutive_deviation_frames=(
                    self.controller.post_stop_out_of_tolerance_streak
                ),
                generation=self.generation,
                reused_yoloe=True,
                detector_reinitialized=False,
            )
            self.logger(
