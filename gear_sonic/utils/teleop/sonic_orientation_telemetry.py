"""Shared contracts for SONIC target-heading and measured-yaw diagnostics."""
...
def encode_orientation_telemetry(sample: OrientationTelemetrySample) -> str:
    """Serialize one validated orientation sample for the local telemetry PUB."""
...
def decode_orientation_telemetry(
    raw: bytes | str,
) -> OrientationTelemetrySample:
    """Validate and decode one relay orientation-telemetry message."""
    try:
...
class OrientationTracker:
    """Track base yaw in the relay's zero-based heading coordinate system."""
    ...
    def sample(
        ...
        if self._actual_yaw_rad is not None and self._origin_yaw_rad is not None:
            actual_heading = _wrapped(
                self._actual_yaw_rad - self._origin_yaw_rad
            )
...
class LatestOrientationTelemetry:
    """Retain the newest decoded relay sample for per-frame diagnostics."""
