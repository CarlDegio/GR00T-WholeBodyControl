...
class OrbbecSensor(Sensor, SensorServer):
    ...
    def __init__(
        ...
        try:
            ...
            source_intrinsics = self._profile_intrinsics(self._color_profile)
            self._camera_info = {
                **source_intrinsics,
                "source_distortion_model": source_intrinsics["distortion_model"],
                "source_distortion_coeffs": source_intrinsics[
                    "distortion_coeffs"
                ],
                ...
                "depth_image_dim": (
                    list(config.depth_image_dim) if config.enable_depth else None
                ),
                "fps": int(config.fps),
    ...
    @staticmethod
    def _profile_intrinsics(profile: Any) -> dict[str, Any]:
    ...
    def read(self) -> dict[str, Any] | None:
        ...
        if self._orbbec_config.enable_depth:
            ...
            published_profile = color_frame.get_stream_profile()
            calibration = dict(self._camera_info)
            calibration.update(self._profile_intrinsics(published_profile))
            calibration["rgb_undistorted"] = self._color_undistortion_filter is not None
